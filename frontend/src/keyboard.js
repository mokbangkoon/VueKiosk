...
function VKI_buildKeyboardInputs() {
    ...
    if (elem) {
      for (var y = 0, keyid = '', ex; (ex = elem[y++]); ) {
        if (
          ...
          keybut.src = 'https://www.google.com/tia/tia.png';
  ...
  var layouts = 0;
  for (let ktype in this.VKI_layout)
  ...
  this.VKI_buildKeys = function () {
    ...
    for (var x = 0, lyt; (lyt = this.VKI_layout[this.VKI_kt][x++]); ) {
      ...
      for (var y = 0, lkey; (lkey = lyt[y++]); ) {
        ...
        if (this.VKI_deadkeysOn)
          for (let key in this.VKI_deadkey) if (key === lkey[0]) alive = true;
            ...
            td.onclick = function () {
              if (self.VKI_deadkeysOn && self.VKI_dead) {
                if (self.VKI_dead != this.firstChild.nodeValue) {
                  for (let key in self.VKI_deadkey) {
        ...
        switch (lkey[1]) {
          ...
          default:
            ...
            if (this.VKI_deadkeysOn) {
              ...
              for (let key in this.VKI_deadkey)
                if (key === char) {
                  alive = true;
                  break;
                }
...
위 방식대로만 하면 간단할 줄 알았는데 예를 들어 "ㅤㅃㅞㄺ" 이란 문자와 "벡" 이란
문자가 있다면 ㅤㅃㅞㄺ은 Qnpfr 이렇게 5개의 문자가 들어가고 벡은 qpr 이렇게 3개 문자면 되지요.
